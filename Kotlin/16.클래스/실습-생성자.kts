...
// 주생성자 -> 생략할수 있는 모든걸 생략하는 방법 (가장 간소한 버젼)
class User31(val userName: String) {
...
class User6 constructor(name: String) {
    ...
    constructor(name: String, age: Int) : this(name) { //User6( 주생성자 먼저실행)
        println("second")
        this.age = age
    }
...
//  - 초기화블록에서 초기화를 보장해주기 때문 -> 클래스가 생성될 때 초기화블록은 무조건 실행된다.
//  - 초기화블록에 없는 속성은 선언할 때 초기화해줘야 한다.
